...
// Fails if the test handle marker (`__GAME__`, see TR §14 / GDD §15.2 / task 05) appears
...
for (const file of walk(dir)) {
  ...
  try {
    ...
    // Node's utf8 decode never throws (invalid byte sequences become U+FFFD replacement
    // characters, even for binary assets) — this only catches a filesystem-level failure, e.g.
    // a file removed between readdir and readFile, or a permission error. Treat it as not a
    // source of the marker and move on.
    continue;
  }
...
if (hits.length > 0) {
  ...
  console.error(
    `\n"${MARKER}" must not appear in a production build (TR §14 / GDD §15.2). ` +
