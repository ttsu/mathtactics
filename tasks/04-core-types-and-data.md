...
- **Tile color/starred fields** (GDD §9.1 "color key"): `tiles.json` entries carry `color: 'green'|'blue'|'orange'` plus a `starred: boolean` (true for `×6`–`×10`) rather than a fourth color value, per the task's own suggested call. `TileDef` in `/sim/core/types.ts` (TR §4) is unchanged (`id, kind, n, priceCategory` only) — `color`/`starred` are data-file-only fields consumed by presentation later.
  **Correction (final M0 whole-branch review, finding 3):** the claim above that "the Zod-inferred tile type is a structural superset of `TileDef`, so it still satisfies `TileDef`" was false as shipped — `z.string()` infers `id: string`, which is *not* assignable to `TileDef.id: TileId` (a template-literal type), so `applyTile(v, gameData.tiles[0])` failed to typecheck (TS2345). Fixed in the M0 final-review fix pass: `TileDefSchema` now ends in `.transform((tile) => ({ ...tile, id: tile.id as TileId }))` after the existing `superRefine` (which already rejects any `id` not exactly `${kind}:${n}`, so the cast is sound on a successful parse), narrowing the inferred `id` to `TileId`. A type-level test (`tests/sim/data/load.test.ts`) now assigns a loaded tile to `TileDef` and passes it to `applyTile` so a regression fails `typecheck` again.
