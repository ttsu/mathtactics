...
- [x] No timing constants — all beat durations, sub-animation shares (`playback.beatShares`), scales, distances, spin angles, shake intensities, star counts and the bar-overshoot cap in `presentation.json` (`pacing` + new `playback` block, schema in `sim/data/schemas.ts`). Code only holds geometry (`layout.ts`) and placeholder colours (`palette.ts`).
...
- **Bounce-back HP display (controller ruling, fix round 1):** the HP **text** only shows payload values — on an overshoot it holds `hpBefore` through the hit while the **bar** dips toward empty, then the `RobotBouncedBack` beat counts the text to `hpAfter` and springs the bar back (Elastic, may overshoot full up to `bounceBack.maxBarFill`) with a robot wobble. For a normal overkill the HP text counts to the (negative) `hpAfter`, shown with a real minus sign (`pieces.formatNumber`, shared with the ball), before the robot pops away.
- **Tile stays readable while it applies (fix round 1):** a ball moving onto a tile it will apply arrives hopped up by `transform.ballHopPt`, and the tile draws above the ball (`DEPTH.liftedTile`) for its beat; the next move brings the ball back down the lane.
- **Abandoned sequences commit nothing (fix round 1):** `Director.stop()` (store reset mid-sequence, or a new sequence replacing an old one) finishes sprites with `finish({ commit: false })`; only a skip or a segment's natural end commits un-played HUD events.
- **Lane wash across gaps (fix round 1):** the dim/highlight wash stays up between lanes and clears only when the whole sequence ends or is abandoned.
- **Scene created mid-playback (fix round 1):** `BoardScene.create` starts the Director if playback is already active.
