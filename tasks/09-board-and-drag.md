...
- **Snap rule:** target = nearest landing cell whose centre is within `snapRadiusCells` (0.75, schema min 0.6) of the piece centre *per axis*, so any point inside a cell counts plus a 25pt margin. The origin cell/slot is a landing cell (drop = no command). Drops resolve at the **held piece's centre** (finger + `fingerOffsetPt` upward), i.e. where the child sees the piece — so a drop with the finger at a blocked cell's centre can snap to an empty cell just above it (the highlight shows this before release).
...
- **Drag tuning in data:** `presentation.json` `drag` { `liftScale`, `liftDurationMs`, `fingerOffsetPt`, `snapRadiusCells`, `settleDurationMs`, `trayScrollThresholdPt` } with schema. Piece/font sizes (`PIECE_SIZE`, `ROBOT_SIZE`, `TRAY_*`, font sizes) are geometry in `layout.ts`; placeholder chrome colours in `game/board/views/palette.ts` (tile face colours still come from `presentation.json`). Tile label text is near-black (≥ 6:1 contrast on all three tile colours); `×6`+ tiles show a small ★ (`tiles.json` `starred`).
...
- **HUD:** End Turn is also disabled with no run or outside planning (not just during playback). Undo shows a `↶` glyph (`aria-label="Undo"`, no reading required, GDD §11.1).
...
- The HUD Undo glyph renders in the system font; replace in the M5 art pass.

**Files created:** `game/board/{BoardScene,BoardRenderer,DragController,dragTargets,pieces,reconcile,trayScroll}.ts`, `game/board/views/{TileView,CannonView,RobotView,palette}.ts`, `game/ui/hudButtons.ts`, `tests/game/{boardFixtures,dragTargets.test,boardPieces.test,reconcile.test,trayScroll.test,hudButtons.test}.ts`, `e2e/board-drag.spec.ts`

**Files modified:** `game/board/BoardLayoutScene.ts` → `game/board/drawBoardBackground.ts`, `game/board/{layout,createBoardGame,index}.ts`, `game/main.tsx`, `game/state/testHandle.ts`, `game/ui/{Hud.tsx,ui.css}`, `data/presentation.json`, `sim/data/schemas.ts`, `tests/game/{layout,testHandle,store}.test.ts`, `tests/sim/data/{load,levels}.test.ts`, `tests/sim/commands/{fixtures.ts,loadLevel.test.ts}` (fixtures gained the `drag` block), `TASKS.md`
