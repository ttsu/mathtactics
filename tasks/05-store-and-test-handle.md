...
- **`applyCommand` fails cleanly when `run` is null**: `dispatch` checks `state.run` before calling `applyCommand` and returns `{ ok: false, error: 'wrong_phase' }` directly (never calls `applyCommand` with a null state), per the task's own decision list.
