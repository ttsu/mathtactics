...
| `/game/state` | `/sim`, `zustand/vanilla`, `@foleyjs/core` (programmatic `play()` only) | `phaser`, `react*` |
...
window.__GAME__ = {
  ...
  getRobotChrome(robotId: string): {        // task 23
    ...
    pairCount: number;                      // identifying marks as drawn: antennae (1) or
                                            // left-hand shield dots (1 = odd blocked, 2 = even blocked)
    coiled: boolean;
...
- Audio: create/resume `AudioContext` on first `pointerdown`. One shared context
  ...
  pops stay bright and melodic. `exactKill` is Foley `complete`. `waveCleared` is
  Foley `success` with per-cue theme `soft` at volume 0.45 (quieter than `win`).
  `getAudioContext()` reuses Foley's context (via
